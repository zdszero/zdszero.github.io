...
def flops_absorb2(q_len, kv_len, verbose=False):
    all_ops = {
        'flops_q_a': q_len * hidden_size * q_lora_rank, # from Q to c_q
        'flops_kv_a': kv_len * hidden_size * (kv_lora_rank + qk_rope_head_dim), # from KV to c_kv and k_pe
...
print(f"decode: absorb vs vanilla ratio  ~ {flops_absorb2(1, kv_len, verbose) / flops_vanilla(1, kv_len, verbose)}")
# print(f"decode: absorb vs absorb2  ~ {flops_absorb(1, kv_len, verbose) / flops_absorb2(1, kv_len, verbose)}")
